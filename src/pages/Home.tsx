...
const Home: React.FC = () => {
  ...
  <main className="mx-auto relative z-10 flex w-full max-w-6xl flex-col px-4 sm:px-6 lg:px-8">
        <section className="milk-hero relative flex min-h-[calc(100vh-7rem)] items-center py-16 sm:py-20 lg:py-24">
          <div className="w-full overflow-hidden rounded-3xl border border-amber-200/60 bg-white/80 px-6 py-12 shadow-xl backdrop-blur-lg transition-colors duration-300 sm:px-10 lg:px-14 dark:border-neutral-800/60 dark:bg-neutral-900/75 dark:shadow-[0_24px_48px_-28px_rgba(15,23,42,0.85)]">
            <div className="grid gap-14 lg:grid-cols-[1.05fr_minmax(0,1fr)] lg:items-center">
              <div className="space-y-10">
                <span className="inline-flex items-center gap-2 rounded-full border border-amber-200/60 bg-amber-50/70 px-4 py-1 text-xs font-semibold uppercase tracking-[0.32em] text-amber-700 transition-colors duration-300 dark:border-neutral-700/70 dark:bg-neutral-800/70 dark:text-amber-300">
                ...
                <div className="space-y-4">
                  ...
                  <p className="max-w-xl text-[15px] leading-relaxed text-neutral-600 sm:text-base dark:text-neutral-300">
                ...
                <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
                  <Link
                    to="/register"
                    className="inline-flex items-center justify-center rounded-xl bg-neutral-900 px-7 py-3 text-sm font-medium text-white shadow-sm transition hover:bg-neutral-800 focus:outline-none focus:ring-2 focus:ring-neutral-900 focus:ring-offset-2 focus:ring-offset-white dark:bg-amber-400 dark:text-neutral-900 dark:hover:bg-amber-300"
                  >
                    Get started
                  </Link>
                  <Link
                    to="/login"
                    className="inline-flex items-center justify-center rounded-xl border border-amber-300 bg-amber-100/80 px-7 py-3 text-sm font-medium text-amber-800 shadow-sm transition hover:bg-amber-200/80 focus:outline-none focus:ring-2 focus:ring-amber-400 focus:ring-offset-2 focus:ring-offset-white dark:border-amber-300/50 dark:bg-neutral-900/40 dark:text-amber-200 dark:hover:bg-neutral-900/20 dark:focus:ring-amber-200 dark:focus:ring-offset-neutral-900"
                  >
                    Sign in
                  </Link>
                </div>
                <ul className="mt-10 grid gap-4 text-sm text-neutral-600 sm:grid-cols-2 dark:text-neutral-300">
                  {heroHighlights.map(item => (
                    <li key={item} className="flex items-start gap-3">
                      <span className="mt-1 flex h-5 w-5 items-center justify-center rounded-full bg-amber-100 text-amber-600 dark:bg-neutral-800 dark:text-amber-300">
              ...
              <div className="space-y-6 lg:pl-12">
                <div className="grid gap-4 sm:grid-cols-2">
                  {heroFeatureBlocks.map(feature => (
                    <article
                      key={feature.title}
                      className="rounded-2xl border border-amber-200/60 bg-white/90 p-5 shadow-sm backdrop-blur-md transition-colors duration-300 sm:p-6 dark:border-neutral-800/60 dark:bg-neutral-900/70"
                    >
                      <span className="inline-flex h-10 w-10 items-center justify-center rounded-xl border border-amber-200/70 bg-amber-100 text-amber-600 transition-colors duration-200 dark:border-neutral-700 dark:bg-neutral-800 dark:text-amber-200">
                        {feature.icon}
                      </span>
                      <p className="mt-3 text-sm font-semibold text-neutral-900 dark:text-neutral-100">{feature.title}</p>
                      <p className="mt-2 text-xs text-neutral-600 dark:text-neutral-300">{feature.description}</p>
                    </article>
                  ))}
                </div>
                <div className="rounded-3xl border border-dashed border-amber-300/70 bg-amber-100/60 p-5 text-sm text-amber-800 transition-colors duration-300 dark:border-amber-400/70 dark:bg-amber-400/10 dark:text-amber-200">
                  Bring your key features to the spotlight—inventory, multi-branch POS, AI analysis, automated reports, and GCash integration all live on the homepage.
                </div>
              </div>
            </div>
          </div>
        ...
        <section id="features" className="scroll-mt-24 py-16 sm:py-20 lg:py-24">
          <div className="mx-auto max-w-3xl text-center space-y-4">
            <h2 className="text-3xl font-semibold text-neutral-900 sm:text-[34px] dark:text-neutral-100">
          ...
          <div className="mt-12 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {featureCards.map(feature => (
                  <article
                key={feature.title}
                className="group relative flex h-full flex-col gap-5 overflow-hidden rounded-3xl border border-neutral-200/70 bg-white/80 p-6 shadow-sm transition-all duration-300 hover:-translate-y-1 hover:border-neutral-300 hover:shadow-lg sm:p-7 dark:border-neutral-800/60 dark:bg-neutral-900/70 dark:hover:border-neutral-700 dark:hover:bg-neutral-900/60"
              >
                <div className="relative flex h-full flex-col gap-4">
                  <span className="inline-flex h-12 w-12 items-center justify-center rounded-xl border border-neutral-200/70 bg-amber-100 text-amber-600 shadow-sm transition-colors duration-200 group-hover:border-amber-300 group-hover:bg-amber-200 group-hover:text-amber-700 dark:border-neutral-700 dark:bg-neutral-800 dark:text-amber-200 dark:group-hover:border-amber-300/60 dark:group-hover:bg-neutral-700 dark:group-hover:text-amber-200">
                    {feature.icon}
                  </span>
                  <div className="space-y-3">
                    <h3 className="text-xl font-semibold text-neutral-900 transition group-hover:text-neutral-800 dark:text-neutral-100 dark:group-hover:text-neutral-50">{feature.title}</h3>
                    <p className="text-[15px] text-neutral-600 transition group-hover:text-neutral-700 dark:text-neutral-300 dark:group-hover:text-neutral-200">{feature.summary}</p>
                  </div>
                  <ul className="mt-auto space-y-2 text-sm text-neutral-600 dark:text-neutral-300">
                    {feature.points.map(point => (
                      <li key={point} className="flex items-start gap-2">
                        <span className="mt-1 inline-block h-2 w-2 rounded-full bg-amber-400/80 dark:bg-amber-300" aria-hidden />
                        <span>{point}</span>
                      </li>
                    ))}
                  </ul>
        ...
        <section id="about" className="scroll-mt-24 py-16 sm:py-20 lg:py-24">
          <div className="grid gap-14 lg:grid-cols-[1.05fr_minmax(0,1fr)] lg:items-start">
            <div className="space-y-6">
              ...
              <ul className="space-y-3 text-sm text-neutral-600 dark:text-neutral-300">
                <li className="flex items-start gap-3">
                  <span className="mt-1 inline-flex h-6 w-6 items-center justify-center rounded-full bg-amber-100 text-amber-600 dark:bg-neutral-800 dark:text-amber-300">
                    <svg viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="1.8" className="h-3.5 w-3.5">
                      <path d="M5 10l3 3 7-7" strokeLinecap="round" strokeLinejoin="round" />
                    </svg>
                  </span>
                  <span>Real-time variance alerts surface void spikes and unusual discounts immediately.</span>
                </li>
                <li className="flex items-start gap-3">
                  <span className="mt-1 inline-flex h-6 w-6 items-center justify-center rounded-full bg-amber-100 text-amber-600 dark:bg-neutral-800 dark:text-amber-300">
                    <svg viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="1.8" className="h-3.5 w-3.5">
                  ...
                  <span>Branch scorecards benchmark crew performance and highlight top sellers every week.</span>
                </li>
                <li className="flex items-start gap-3">
                  <span className="mt-1 inline-flex h-6 w-6 items-center justify-center rounded-full bg-amber-100 text-amber-600 dark:bg-neutral-800 dark:text-amber-300">
                    <svg viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="1.8" className="h-3.5 w-3.5">
                  ...
                  <span>Ingredient usage, prep batches, and cash reconciliation live side by side.</span>
                </li>
              </ul>
            </div>
            <div className="grid gap-5 sm:grid-cols-2">
              {branchHighlights.map(highlight => (
                <article
                  key={highlight.title}
                  className="rounded-3xl border border-neutral-200/70 bg-white/80 p-6 shadow-sm transition-colors duration-300 dark:border-neutral-800/60 dark:bg-neutral-900/70"
                >
                  <p className="text-sm font-semibold text-neutral-900 dark:text-neutral-100">{highlight.title}</p>
        ...
        <section id="contact" className="scroll-mt-24 py-16 sm:py-20 lg:py-24">
          <div className="mx-auto flex max-w-5xl flex-col gap-8 rounded-3xl border border-neutral-200/70 bg-white/80 p-8 shadow-lg transition-colors duration-300 sm:gap-10 sm:p-10 md:flex-row dark:border-neutral-800/60 dark:bg-neutral-900/70 dark:shadow-[0_24px_48px_-28px_rgba(15,23,42,0.85)]">
            <div className="flex-1 space-y-4">
              <h2 className="text-2xl font-semibold text-neutral-900 sm:text-[28px] dark:text-neutral-100">Ready for a calmer café rollout?</h2>
              <p className="text-[15px] text-neutral-600 dark:text-neutral-300">
                Our team guides menu migration, payment setup, and branch training so you can focus on customer experience.
              </p>
              <Link
                to="/register"
                className="inline-flex w-full max-w-xs items-center justify-center rounded-xl bg-neutral-900 px-6 py-3 text-sm font-medium text-white transition hover:bg-neutral-800 focus:outline-none focus:ring-2 focus:ring-neutral-900 focus:ring-offset-2 focus:ring-offset-white dark:bg-amber-400 dark:text-neutral-900 dark:hover:bg-amber-300 dark:focus:ring-amber-200 dark:focus:ring-offset-neutral-900"
              >
                Book a walkthrough
              </Link>
            </div>
            <div className="flex-1 space-y-5 text-sm text-neutral-600 dark:text-neutral-300">
              <div>
                <p className="text-xs font-semibold uppercase tracking-[0.22em] text-neutral-500 dark:text-neutral-400">Email</p>
                <p className="mt-1 text-lg font-semibold text-neutral-900 dark:text-neutral-100">hello@kaperest.io</p>
                <p className="mt-2">Send us your current branch setup—we’ll map the rollout within a day.</p>
              </div>
              <div>
                <p className="text-xs font-semibold uppercase tracking-[0.22em] text-neutral-500 dark:text-neutral-400">Support line</p>
                <p className="mt-1 text-lg font-semibold text-neutral-900 dark:text-neutral-100">+63 917 000 1234</p>
                <p className="mt-2">Weekdays 9 AM–6 PM for deployment planning and support.</p>
              </div>
            </div>
          </div>
        </section>
