...
const LoginUI: React.FC = () => {
  ...
  return (
    <div className="relative flex min-h-[100dvh] flex-col overflow-hidden bg-transparent font-sans">
      <TintedBackdrop />
      {/* page overlay: white translucent with a subtle blue tint (~5%) */}
  ...
  <div className="relative w-full max-w-[380px]">
    <div className="auth-card relative rounded-3xl border border-amber-200/70 bg-white/80 p-6 shadow-sm backdrop-blur-lg transition-all duration-300 ease-out hover:-translate-y-2 hover:shadow-xl focus-within:-translate-y-2 dark:border-neutral-800/70 dark:bg-neutral-900/85">
      <div className="absolute -top-3 left-6 inline-flex h-6 items-center rounded-full border border-amber-200 bg-amber-50/80 px-3 text-[11px] font-medium tracking-wide text-amber-600 dark:border-amber-400/60 dark:bg-amber-400/15 dark:text-amber-200">
              LOGIN
            </div>
          {/* Brand */}
          <div className="mb-6">
            <h1 className="text-[27px] font-semibold tracking-tight text-neutral-900 dark:text-neutral-100 flex items-center gap-2">KapeRest <span aria-hidden="true" className="coffee-emoji text-[20px]">☕</span></h1>
            <p className="mt-1 text-sm text-neutral-500 dark:text-neutral-400">Access your POS workspace</p>
          </div>
          ...
          {/* Form */}
            <form onSubmit={handleSubmit} noValidate className="space-y-5" aria-busy={isLoading}>
              ...
              {errorSummary && (
                <div className="rounded-lg border border-neutral-300 bg-neutral-50 text-neutral-700 text-[12px] px-3 py-2 dark:border-neutral-700 dark:bg-neutral-900/70 dark:text-neutral-200" role="alert">
                  {errorSummary}
                </div>
              )}
              <div className="space-y-5">
                ...
                <div className="relative">
                    ...
                    <input
                      ...
                      className={`peer block w-full rounded-xl border bg-neutral-50/80 pr-10 px-3.5 py-3 text-[15px] leading-tight tracking-tight text-neutral-900 focus:outline-none transition placeholder:text-neutral-400 dark:bg-neutral-900/60 dark:text-neutral-100 dark:placeholder:text-neutral-500
                        ${emailValid && email.trim().length > 0
                          ...
                          : 'border-neutral-300 focus:ring-1 focus:ring-neutral-900 focus:border-neutral-900 dark:border-neutral-700 dark:focus:ring-amber-400 dark:focus:border-amber-400'}
                  ...
                  {errors.email && (
                    <p id="email-error" className="mt-1 text-[11px] text-neutral-600 dark:text-neutral-300">{errors.email}</p>
                  )}
                </div>
                ...
                <div className="relative">
                  <label htmlFor="password" className="flex items-center justify-between text-[13px] font-medium text-neutral-700 tracking-wide dark:text-neutral-300">
                    <span>Password</span>
                    {capsOn && <span className="text-[10px] font-medium text-amber-500 dark:text-amber-300">CAPS ON</span>}
                  </label>
                  <div className="mt-1 relative">
                    <input
                      id="password"
                  ...
                  {errors.password && (
                    <p id="password-error" className="mt-1 text-[11px] text-neutral-600 dark:text-neutral-300">{errors.password}</p>
                  )}
                </div>
              </div>
              <div className="flex items-center justify-between">
                <label className="group relative inline-flex items-center gap-2 pl-[2px] text-[11px] font-medium text-neutral-700 select-none leading-none cursor-pointer dark:text-neutral-300">
                  <input
                    type="checkbox"
                    disabled={isLoading}
                    className="peer relative h-4 w-4 shrink-0 appearance-none rounded-[5px] border border-neutral-300 bg-white transition-colors before:absolute before:inset-0 before:rounded-[4px] before:bg-neutral-900 before:scale-0 before:opacity-0 before:transition checked:before:scale-100 checked:before:opacity-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-neutral-900 disabled:opacity-60 cursor-pointer dark:border-neutral-700 dark:bg-neutral-800 dark:before:bg-amber-400 dark:focus-visible:ring-amber-300"
                  />
          ...
          {/* Secondary */}
          <p className="mt-6 text-center text-[13px] text-neutral-600 dark:text-neutral-300">
           Dont have an account? <Link to="/register" className="font-medium text-neutral-800 hover:underline focus:outline-none focus:ring-2 focus:ring-neutral-900 focus:ring-offset-2 focus:ring-offset-white rounded dark:text-amber-300 dark:focus:ring-amber-200 dark:focus:ring-offset-neutral-900">Sign up</Link>
          </p>
          </div>
        </div>
      </main>
