...
import { faCoffee, faSearch, faBars, faCalendarDays, faWeightScale, faCalendarAlt, faReceipt, faMoneyBillWave, faChartLine, faSpinner, faSun, faCalendar, faBuilding } from '@fortawesome/free-solid-svg-icons';
  ...
  const handleGenerateReport = async (period: PeriodFilter) => {
    ...
    try {
      ...
      if (contentDisposition) {
        ...
        if (period === 'daily') fileName = 'AdminDailySalesReport.pdf';
        if (period === 'yearly') fileName = 'AdminYearlySalesReport.pdf';
        if (period === 'monthly') fileName = 'AdminMonthlySalesReport.pdf';
      }
      ...
      if (msg.includes('Failed to fetch') || msg.includes('NetworkError')) {
        setError('Failed to connect to the backend API. Ensure the backend is running and the base URL is correct.');
      } else {
        setError(msg);
      }
      ...
      <div className="sticky top-0 z-30 backdrop-blur-xl bg-white/95 dark:bg-neutral-900/95 border-b border-orange-200/30 dark:border-orange-900/30 shadow-xl shadow-orange-500/5 transition-all duration-300">
          ...
          <div className="relative group mb-4">
            ...
            <div className="relative flex items-center gap-3 bg-white dark:bg-neutral-800 rounded-2xl border-2 border-orange-200/50 dark:border-orange-900/50 focus-within:border-orange-500 dark:focus-within:border-orange-500 focus-within:shadow-xl focus-within:shadow-orange-500/20 px-5 py-4 shadow-lg hover:shadow-xl transition-all duration-300">
              ...
              />
                <input
                type="text"
                placeholder="Search receipts, item, cashier, branch, status..."
                value={searchText}
        ...
        <div className="flex items-center justify-between gap-4 flex-wrap">
          <div className="flex items-center gap-4 flex-wrap">
            ...
            {/* Branch Filter */}
            <div className="relative group">
              <div className="absolute inset-0 bg-gradient-to-r from-orange-500/20 to-amber-500/20 rounded-2xl blur-lg opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>
              <div className="relative flex items-center gap-3 px-5 py-3 rounded-2xl bg-white dark:bg-neutral-800 border-2 border-orange-200/50 dark:border-orange-900/50 hover:border-orange-400 dark:hover:border-orange-600 transition-all shadow-lg hover:shadow-xl">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex gap-2.5">
              {[
                { t: 'daily', icon: faSun, label: 'Daily' }, 
                { t: 'monthly', icon: faCalendar, label: 'Monthly' }, 
                { t: 'yearly', icon: faCalendarAlt, label: 'Yearly' }
              ].map(({ t, icon, label }) => (
                <button
                  key={t}
                  onClick={() => handleGenerateReport(t as PeriodFilter)}
            ...
            {loading ? (
              <div className="flex h-full items-center justify-center py-24">
                <div className="text-center px-4 max-w-md">
                  ...
                  <h4 className="text-2xl font-black text-stone-900 dark:text-white mb-3">Loading sales...</h4>
                  <p className="text-base font-medium text-stone-600 dark:text-stone-400 leading-relaxed mb-6">Please wait while we retrieve sales data.</p>
                </div>
              </div>
            ) : error ? (
              <div className="flex h-full items-center justify-center py-24">
                <div className="text-center px-4 max-w-md">
                  ...
                  <h4 className="text-2xl font-black text-stone-900 dark:text-white mb-3">Error loading sales</h4>
                  <p className="text-base font-medium text-stone-600 dark:text-stone-400 leading-relaxed mb-6">{error}</p>
                </div>
              </div>
                  ...
                  {filteredSales.map((sale) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-5 text-left">
                        <span className="text-sm font-bold text-stone-900 dark:text-white">{sale.menuItemName || '—'}</span>
                      </td>
                      <td className="px-6 py-5 text-left hidden sm:table-cell">
                        <span className="text-sm text-stone-700 dark:text-stone-300">{sale.cashierName ?? '—'}</span>
                      </td>
                      <td className="px-6 py-5 text-left hidden lg:table-cell">
                        <div className="flex flex-col">
                          <span className="text-sm font-semibold text-stone-700 dark:text-stone-300">{sale.branchName ?? '—'}</span>
                          {sale.branchLocation && (
                            <span className="text-xs text-stone-500 dark:text-stone-400">{sale.branchLocation}</span>
                          )}
                        </div>
                      </td>
                      <td className="px-6 py-5">
                        <span className="text-sm font-semibold text-stone-700 dark:text-stone-300">{formatDate(sale.dateTime)}</span>
                      </td>
